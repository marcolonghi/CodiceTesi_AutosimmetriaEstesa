...
int main(int argc, char** argv)
{
    ...
    //Conto il numero di elementi di u (onset \cup dcset di f)
    count = Cudd_ApaCountMinterm(manager,u,f->inputs,&digits);
    ...
    if(degree>=1 && degree<=f->inputs)
    {
        ...
        fk = restrictionFunction(u, cv[0], f->inputs);
